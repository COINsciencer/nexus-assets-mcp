...
if (!TOKEN) {
  process.stderr.write(
    '[nexus-assets-mcp] ⚠️  NEXUS_TOKEN not set.\n' +
...
async function handleGetPortfolio(args: Record<string, unknown>): Promise<string> {
  const data = await api<any>(`/aggregate${args.refresh ? '?refresh=1' : ''}`)
  const lines = [
    '## Portfolio Summary',
    ...
    '',
    '### By Category',
    ...(data.categories ?? []).map((c: any) => `- **${c.label}**: ¥${fmtCny(c.value)} (${c.pct?.toFixed(1)}%)`),
    '',
    '### Top Assets',
    ...(data.assetBreakdown ?? []).slice(0, 15).map((a: any) => `- **${a.label}**: ${a.amount}  |  ¥${fmtCny(a.valueCny)}`),
  ]
  ...
  if (data.cache?.cached) lines.push('', `*Cached data, ${data.cache.ageSec}s old — pass refresh=true for live data*`)
  return lines.join('\n')
}

async function handleGetDashboardScreenshot(): Promise<string> {
...
async function handleListAccounts(): Promise<string> {
  const [accounts, summary] = await Promise.all([api<any[]>('/accounts'), api<any[]>('/accounts/summary')])
  const sumMap = new Map((summary ?? []).map((s: any) => [s.account_id, s]))
...
async function handleGetAccountAssets(args: Record<string, unknown>): Promise<string> {
  const data = await api<any>(`/accounts/${args.account_id}/assets`)
  const lines = [
    ...
    '| Asset | Amount | CNY |',
    '|-------|--------|-----|',
    ...(data.assets ?? []).map((a: any) => `| ${a.asset_id} | ${a.amount} | ¥${fmtCny(a.value_cny ?? 0)} |`),
  ]
  return lines.join('\n')
}

async function handleRefreshAccount(args: Record<string, unknown>): Promise<string> {
  const data = await api<any>(`/accounts/${args.account_id}/refresh`, { method: 'POST' })
  return `✅ Refreshed \`${args.account_id}\`\nTotal: ¥${fmtCny(data.total_cny)}  Updated: ${data.updated_at}`
}

async function handleGetAssetPrice(args: Record<string, unknown>): Promise<string> {
  const id = String(args.asset_id).toUpperCase()
  const d = await api<any>(`/pricing/${id}`)
...
async function handleGetPortfolioTrend(args: Record<string, unknown>): Promise<string> {
  const days = Math.max(1, Math.min(365, Number(args.days ?? 30)))
  const data = await api<any>(`/trend/portfolio?days=${days}`)
  const pts: any[] = data.points ?? []
  if (!pts.length) return 'No trend data yet.'
  ...
  const lines = [
    ...
    '| Time | CNY | USD |',
    '|------|-----|-----|',
    ...pts.slice(-10).map((p: any) => `| ${p.ts.slice(0, 16).replace('T', ' ')} | ¥${fmtCny(p.total_cny)} | $${fmtUsd(p.total_usd)} |`),
  ]
  return lines.join('\n')
}

async function handleGetSnapshots(args: Record<string, unknown>): Promise<string> {
  const limit = Math.min(90, Math.max(1, Number(args.limit ?? 30)))
  const data = await api<any[]>(`/snapshots?limit=${limit}`)
  if (!data?.length) return 'No snapshots yet.'
  const lines = [
    `## Daily Snapshots (${data.length})`,
    '| Date | Total CNY | Change |',
    '|------|-----------|--------|',
    ...data.map((s: any) => `| ${s.date ?? s.snapshot_at?.slice(0, 10)} | ¥${fmtCny(s.total_cny ?? s.total ?? 0)} | ${(s.change_pct ?? s.change ?? 0) >= 0 ? '+' : ''}${(s.change_pct ?? s.change ?? 0).toFixed(2)}% |`),
  ]
  return lines.join('\n')
}

async function handleListManualAssets(): Promise<string> {
  const data = await api<any[]>('/manual-assets')
  if (!data?.length) return 'No manual assets.'
  const lines = [
    '## Manual Assets',
    '| ID | Asset | Amount | Note |',
    '|----|-------|--------|------|',
    ...data.map((a: any) => `| \`${a.id}\` | ${a.asset_id} | ${a.amount} | ${a.note ?? ''} |`),
  ]
  return lines.join('\n')
}

async function handleAddManualAsset(args: Record<string, unknown>): Promise<string> {
  const accounts = await api<any[]>('/accounts')
  const manual = accounts?.find((a: any) => a.type === 'manual')
  if (!manual) return '❌ No manual assets account found. Please create one in the app first.'
  ...
  if (args.value_usd != null) body.value_usd = Number(args.value_usd)
  const result = await api<{ id: string }>('/manual-assets', { method: 'POST', body: JSON.stringify(body) })
  return `✅ Added manual asset \`${body.asset_id}\` × ${body.amount}  (id: \`${result.id}\`)`
}

async function handleUpdateManualAsset(args: Record<string, unknown>): Promise<string> {
  const updates: Record<string, unknown> = {}
  if (args.asset_id != null) updates.asset_id = String(args.asset_id).toUpperCase()
  if (args.amount != null) updates.amount = Number(args.amount)
  if (args.note != null) updates.note = String(args.note)
  if (args.value_usd != null) updates.value_usd = Number(args.value_usd)
  await api(`/manual-assets/${args.id}`, { method: 'PATCH', body: JSON.stringify(updates) })
  return `✅ Updated asset \`${args.id}\``
}

async function handleDeleteManualAsset(args: Record<string, unknown>): Promise<string> {
  await api(`/manual-assets/${args.id}`, { method: 'DELETE' })
  return `✅ Deleted asset \`${args.id}\``
}

async function handleAddAccount(args: Record<string, unknown>): Promise<string> {
  const result = await api<{ id: string }>('/accounts', {
    method: 'POST',
...
async function handleSaveCredentials(args: Record<string, unknown>): Promise<string> {
  const body: Record<string, string> = {
    api_key: String(args.api_key),
    secret: String(args.secret),
  }
  if (args.passphrase) body.passphrase = String(args.passphrase)
...
function fmtCny(v: number): string { return Number(v ?? 0).toLocaleString('zh-CN', { maximumFractionDigits: 2 }) }
function fmtUsd(v: number): string { return Number(v ?? 0).toLocaleString('en-US', { maximumFractionDigits: 2 }) }
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    let text: string
    switch (name) {
      case 'get_portfolio':            text = await handleGetPortfolio(args); break
      case 'get_dashboard_screenshot': text = await handleGetDashboardScreenshot(); break
      case 'list_accounts':            text = await handleListAccounts(); break
      case 'get_account_assets':       text = await handleGetAccountAssets(args); break
      case 'refresh_account':          text = await handleRefreshAccount(args); break
      case 'get_asset_price':          text = await handleGetAssetPrice(args); break
      case 'get_portfolio_trend':      text = await handleGetPortfolioTrend(args); break
      case 'get_snapshots':            text = await handleGetSnapshots(args); break
      case 'list_manual_assets':       text = await handleListManualAssets(); break
      case 'add_manual_asset':         text = await handleAddManualAsset(args); break
      case 'update_manual_asset':      text = await handleUpdateManualAsset(args); break
      case 'delete_manual_asset':      text = await handleDeleteManualAsset(args); break
      case 'add_account':              text = await handleAddAccount(args); break
      case 'save_account_credentials': text = await handleSaveCredentials(args); break
      default:
